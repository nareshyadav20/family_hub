...
const getIconData = (type) => {
  switch(type) {
    ...
    case 'member':
    case 'join': return { icon: <Users size={16} />, iconColor: 'bg-blue-100 text-blue-600 ' };
    ...
    default: return { icon: <Bell size={16} />, iconColor: 'bg-slate-100 text-slate-600 ' };
  }
};
...
export default function Notifications() {
  ...
  const markRead = (id, currentReadStatus) => {
     if (!currentReadStatus) {
         ...
         // Storing in localstorage helps ensure cross-device read states if DB isn't tracking per user
      ...
      <div className="flex gap-2 flex-wrap">
        {tabs.map(tab => (
          <button key={tab.key} onClick={() => { setFilter(tab.key); setCurrentPage(1); }}
            className={`px-3.5 py-1.5 rounded-full text-xs font-bold transition-all ${filter === tab.key ? 'bg-blue-600 text-white shadow-md shadow-blue-500/30' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'}`}>
            {tab.label}
          </button>
        ))}
      </div>
        ...
        {currentItems.map(notif => {
          ...
          return (
            <div key={notif.id} onClick={() => markRead(notif.id, notif.read)}
              className={`flex items-start gap-4 p-4 rounded-2xl cursor-pointer transition-all ${notif.read ? 'bg-white border border-slate-100 ' : 'bg-blue-50 border border-blue-100 '}`}>
