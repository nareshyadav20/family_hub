    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 h-10 rounded-xl bg-white border border-slate-200 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#7C5CFC]/20"
          />
        </div>
        <div className="flex gap-2 flex-wrap">
          {tags.map(tag => (
            <button key={tag} onClick={() => setActiveTag(tag)}
              className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all ${activeTag === tag ? 'bg-[#7C5CFC] text-white shadow-md shadow-[#7C5CFC]/20' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'}`}
            >{tag}</button>
          ))}
        </div>
      </div>
      ...
      {!isLoading && filtered.length === 0 && (
         <div className="py-16 text-center bg-[#FAF8FF] rounded-3xl border border-dashed border-[#E9E5F8] w-full mt-4">
            ...
            <p className="text-sm text-slate-500 max-w-sm mx-auto mb-6">Preserve and share your family's most precious memories. Upload your first photo to start the gallery.</p>
              ...
              <label className="border-2 border-dashed border-slate-300 rounded-xl p-10 flex flex-col items-center justify-center bg-slate-50 cursor-pointer hover:bg-slate-100 :bg-slate-800 transition-colors w-full relative">
                 ...
                 {uploadPreview ? (
                     <div className="text-center">
                        ...
                        <span className="text-sm font-bold text-slate-600 ">{selectedFile?.name}</span>
                     </div>
                 ) : (
                     <div className="text-center flex flex-col items-center">
                        <Upload size={40} className="text-[#7C5CFC] mb-3" />
                        <span className="text-sm font-bold text-slate-600 mb-1">No file chosen</span>
                        <span className="text-sm font-bold text-slate-600 ">Browse or drop photos here</span>
