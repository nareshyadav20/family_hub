...
export default function Documents() {
  ...
  const getIcon = (type) => {
    ...
    return <FileText size={20} className="text-[#7C5CFC]" />;
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-[#1F2430]">Family Documents</h1>
          <p className="text-slate-500 text-sm mt-1">Access verified documents or upload new files securely.</p>
        </div>
        <button onClick={() => setShowModal(true)} className="flex items-center gap-2 bg-[#7C5CFC] hover:bg-blue-700 text-white px-5 py-2.5 rounded-[24px] font-semibold transition-all shadow-lg shadow-blue-500/30">
          <Upload size={18} /> Upload Document
        </button>
      </div>

      <div className="bg-white p-6 rounded-3xl border border-[#E9E5F8] shadow-sm">
         {/* Folder Grid */}
           ...
           {folders.map(f => {
             ...
             return (
               <button key={f} onClick={() => setActiveFolder(f)}
                 className={`flex items-center gap-2 px-5 py-3 rounded-[24px] border transition-all whitespace-nowrap ${activeFolder === f ? 'bg-[#7C5CFC] border-blue-600 text-white shadow-sm shadow-blue-500/20' : 'bg-[#FCFBFF] border-[#E9E5F8] text-slate-600 hover:border-blue-300'}`}>
                 <Folder size={16} className={activeFolder === f ? 'text-blue-200' : 'text-amber-400'} />
                 <span className="text-sm font-bold">{f}</span>
                 <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${activeFolder === f ? 'bg-blue-700/50 text-blue-100' : 'bg-slate-200 text-slate-500 '}`}>{count}</span>
               </button>
             );
           })}
         </div>
         ...
         <div className="relative mb-6">
           ...
           <input type="text" placeholder="Search for documents..." value={search} onChange={e => setSearch(e.target.value)}
             className="w-full pl-11 pr-4 py-3 rounded-[24px] bg-[#FCFBFF] border border-[#E9E5F8] text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#7C5CFC]/30" />
         </div>

         {/* File list */}
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
           {filtered.map(doc => (
             <div key={doc.id} className="bg-white rounded-2xl border border-[#E9E5F8] p-5 flex items-center gap-4 hover:shadow-sm transition-shadow group relative overflow-hidden">
               <div className="w-12 h-12 rounded-[24px] bg-[#FCFBFF] flex items-center justify-center shrink-0">
                 {getIcon(doc.type)}
               </div>
               <div className="flex-1 min-w-0">
                 <div className="flex items-center gap-2 mb-1">
                   <p className="font-bold text-[#1F2430] text-[15px] truncate">{doc.name}</p>
                   {doc.visibility === 'PRIVATE' && <Lock size={12} className="text-amber-500 shrink-0" />}
                 </div>
                 <div className="flex items-center gap-2 text-xs font-medium text-slate-500">
                    <span className="px-2 py-0.5 rounded-md bg-[#FAF8FF] text-slate-600 font-bold">{doc.category}</span>
                    <span>• {doc.size}</span>
                 </div>
               </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 p-6 relative">
              <h2 className="text-xl font-bold text-[#1F2430] mb-4">Upload Secure Document</h2>
              ...
              <div className="space-y-4">
                <input 
                  ...
                  className="w-full bg-[#FCFBFF] border border-[#E9E5F8] rounded-[24px] px-4 py-3 text-sm font-medium focus:ring-2 focus:ring-[#7C5CFC]/50" 
                />
                <div className="flex gap-4">
                   <select 
                      value={uploadData.category}
                      onChange={e => setUploadData({...uploadData, category: e.target.value})}
                      className="w-1/2 bg-[#FCFBFF] border border-[#E9E5F8] rounded-[24px] px-4 py-3 text-sm font-medium">
                ...
                <label className="border-2 border-dashed border-slate-300 rounded-[24px] p-8 flex flex-col items-center justify-center bg-[#FCFBFF] cursor-pointer hover:bg-[#FAF8FF] transition-colors w-full relative">
                   ...
                   {selectedFile ? (
                     ...
                     <div className="text-center flex flex-col items-center">
                        <Upload size={32} className="text-[#7C5CFC] mb-3" />
                        <span className="text-sm font-bold text-slate-600">Browse or drop document here</span>
              ...
              <div className="mt-5 flex gap-3">
                 <button 
                  ...
                  className="flex-1 bg-[#7C5CFC] hover:bg-blue-700 disabled:opacity-50 text-white py-3 rounded-[24px] font-bold text-sm transition-colors shadow-sm shadow-blue-500/20">
                    {uploadDoc.isPending ? 'Uploading...' : 'Secure Upload'}
                 </button>
                 <button onClick={() => setShowModal(false)} className="px-6 bg-[#FAF8FF] hover:bg-slate-200 text-slate-700 border border-[#E9E5F8] py-3 rounded-[24px] font-bold text-sm transition-colors">Cancel</button>
              </div>
           </div>
        </div>
      )}
