  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-[#1F2430]">Family Events</h1>
          <p className="text-slate-500 font-medium text-sm mt-1">Discover upcoming family gatherings and celebrations.</p>
        </div>
      </div>

      <div className="flex gap-4 border-b border-[#E9E5F8] pb-px">
        <button onClick={() => setActiveTab('upcoming')} className={`pb-3 border-b-2 font-bold px-2 transition-colors ${activeTab === 'upcoming' ? 'border-indigo-600 text-[#7C5CFC]' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>Upcoming ({upcomingEvents.length})</button>
        <button onClick={() => setActiveTab('past')} className={`pb-3 border-b-2 font-medium px-2 transition-colors ${activeTab === 'past' ? 'border-indigo-600 text-[#7C5CFC] font-bold' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>Past Events ({pastEvents.length})</button>
      </div>

      {isLoading ? (
        <div className="py-20 text-center font-bold text-slate-400">Loading Events...</div>
      ) : activeEvents.length === 0 ? (
        <div className="py-20 text-center text-slate-400 font-semibold bg-[#FCFBFF] rounded-3xl border border-dashed border-[#E9E5F8]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {activeEvents.map((e) => {
            ...
            return (
              <Card key={e.id} className="border-0 shadow-sm bg-white group cursor-pointer hover:-translate-y-1 hover:shadow-xl transition-all duration-300 overflow-hidden rounded-2xl flex flex-col">
                <div className="h-40 w-full relative overflow-hidden bg-[#FAF8FF] flex items-center justify-center text-slate-300">
                  ...
                  <div className="absolute top-4 left-4 z-20 bg-white/95 backdrop-blur-md rounded-[24px] p-2 shadow-lg text-center min-w-[55px] border border-white/50">
                    <div className="text-[10px] font-black text-[#7C5CFC] uppercase tracking-wider">{month}</div>
                    <div className="text-2xl font-black text-[#1F2430] leading-none mt-0.5">{day}</div>
                  </div>
                ...
                <CardContent className="p-6 flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="text-xl font-bold mb-3 text-[#1F2430] line-clamp-1 group-hover:text-[#7C5CFC] transition-colors">{e.name}</h3>
                  ...
                  <div className="mt-6 pt-5 border-t border-[#E9E5F8] flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-semibold text-slate-500 uppercase tracking-widest">{e.familyBranch} Branch</span>
                    </div>
                    <div className="text-[#7C5CFC] font-bold text-sm flex items-center">
                  ...
                  {(e.liveStream && e.streamId) && (
                    <div className="mt-4 w-full">
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-[#2563EB] text-white rounded-[20px] hover:bg-blue-700 transition font-bold text-sm shadow-sm shadow-blue-500/20"
                      >
