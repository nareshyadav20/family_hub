...
export default function Members() {
  ...
  const liveMembers = rawMembers.map(m => ({
    ...
    role: m.role,
    status: m.status,
    progress: m.status === 'ACTIVE' ? 100 : m.status === 'INVITATION_SENT' ? 20 : 0,
                    ...
                    <td className="px-4 py-4 w-32">
                       <div className="flex items-center gap-2">
                          <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                             <div className={`h-full rounded-full ${m.progress === 100 ? 'bg-emerald-500' : m.progress > 40 ? 'bg-amber-500' : 'bg-rose-500'}`} style={{ width: `${m.progress}%` }}></div>
                          </div>
