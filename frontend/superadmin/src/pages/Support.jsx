...
export default function Support() {
  ...
  const statsData = [
    ...
    { name: 'Closed', value: data.stats.closed?.toString() || '0', icon: MessageSquare, color: 'text-slate-600', bg: 'bg-slate-100' },
  ];
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {data.tickets.map((ticket) => (
                      <tr key={ticket.id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="py-4 px-6 font-bold text-slate-900 text-sm">#{ticket.id.substring(ticket.id.length - 6).toUpperCase()}</td>
