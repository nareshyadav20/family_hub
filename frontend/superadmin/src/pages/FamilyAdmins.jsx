...
export default function FamilyAdmins() {
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Family Admins</h2>
        <p className="text-sm text-gray-500 mt-1">Manage all administrative accounts across families.</p>
      </div>
            ...
            <tbody>
              {defaultAdmins.map(admin => (
                <tr key={admin.id} className="border-b border-gray-50 hover:bg-purple-50/30">
                  ...
                  <td className="px-6 py-4 text-gray-500">{admin.email}<br/><span className="text-xs">{admin.phone}</span></td>
                  <td className="px-6 py-4 text-gray-900">{admin.family}</td>
